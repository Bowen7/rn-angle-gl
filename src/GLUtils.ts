...
export function configureLogging(gl: ExpoWebGLRenderingContext): void {
  ...
  gl.__expoSetLogging = (option: GLLoggingOption): void => {
    ...
    // Turn on logging.
    Object.entries(Object.getPrototypeOf(gl)).forEach(
      ([key, originalValue]) => {
        ...
        gl[key] = (...args) => {
          if (loggingOption & GLLoggingOption.METHOD_CALLS) {
            const params = args.map((arg) => {
              ...
              if (
                loggingOption & GLLoggingOption.RESOLVE_CONSTANTS &&
                typeof arg === "number"
              ) {
              ...
              if (
                loggingOption & GLLoggingOption.TRUNCATE_STRINGS &&
                typeof arg === "string"
              ) {
                if (arg.length > MAX_STRING_LENGTH) {
                  ...
                  return (
                    arg.substr(
                      0,
                      lastIndex >= 0 ? lastIndex : MAX_STRING_LENGTH
                    ) + "..."
                  );
                }
              }
            ...
            console.log(`RnAngleGl: ${key}(${params.join(", ")})`);
          }

          const result = originalValue.apply(gl, args);

          if (loggingOption & GLLoggingOption.METHOD_CALLS) {
            console.log(`RnAngleGl:   = ${result}`);
          }
          if (
            loggingOption & GLLoggingOption.GET_ERRORS &&
            key !== "getError"
          ) {
            ...
            if (error && error !== gl.NO_ERROR) {
              ...
              console.log(`\x1b[31mRnAngleGl: Error ${GLErrors[error]}\x1b[0m`);
            }
          }
