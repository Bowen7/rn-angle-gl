...
export class GLView extends React.Component<GLViewProps> {
   ...
   */
  static async destroyContextAsync(
  ...
  static getWorkletContext: (
    contextId: number
  ) => ExpoWebGLRenderingContext | undefined = workletContextManager.getContext;
  ...
  componentDidUpdate(prevProps: GLViewProps): void {
    if (
      this.props.enableExperimentalWorkletSupport !==
      prevProps.enableExperimentalWorkletSupport
    ) {
      console.warn(
  ...
  async takeSnapshotAsync(options: SnapshotOptions = {}): Promise<GLSnapshot> {
    if (!GLView.takeSnapshotAsync) {
      throw new UnavailabilityError("rn-angle-gl", "takeSnapshotAsync");
    }
    const { exglCtxId } = this;
